...
// entry, and the three places that recognize it (compose() in ui/theme/current.go,
// and requestSchemeCmd and applyDetectedScheme in app/scheme.go) compare against the
// literal. Without normalizing here, "Auto" would be the one theme value in the file
// that silently does nothing: it would render the dark default and never send a
// detection query, on a light terminal, with no error anywhere.
//
// Doing it in this accessor rather than at those three sites is what keeps it one
// fact. All of them reach the config only through GetTheme.
