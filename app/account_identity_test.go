...
func TestStartNewSession_RefusesMismatchedIdentity(t *testing.T) {
	...
	_, _, err := h.startNewSession("s", t.TempDir(), true, false, "echo", "", "", nil, spawnInteractive, nil)
...
func TestStartNewSession_GatesTheRotatedMember(t *testing.T) {
	...
	before := h.list.NumInstances()
	_, _, err := h.startNewSession("s2", t.TempDir(), true, false, "echo", "", "", nil, spawnInteractive, nil)
...
func TestStartNewSession_GatesAPickerPin(t *testing.T) {
	...
	_, _, err := h.startNewSession("s", t.TempDir(), true, false, "echo", "", "", pin, spawnInteractive, nil)
