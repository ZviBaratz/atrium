...
func TestSweepMetadataNowCmdGuards(t *testing.T) {
	...
	require.Nil(t, sweepMetadataNowCmd(context.Background(), nil, selected, 0, usagePolicy{}, 0),
		"no active sessions yields no sweep")
	require.NotNil(t, sweepMetadataNowCmd(context.Background(), []*session.Instance{selected}, selected, 0, usagePolicy{}, 0),
		"the selected row alone still yields a sweep (it is refreshed face-value)")
	require.NotNil(t, sweepMetadataNowCmd(context.Background(), []*session.Instance{selected, other}, selected, 0, usagePolicy{}, 0),
...
func TestTickUpdateMetadataCmdHonorsContextCancellation(t *testing.T) {
	...
	cmd := tickUpdateMetadataCmd(ctx, nil, nil, true, 0, usagePolicy{}, 0)
