...
| `image_preview` | Appearance | string | `"auto"` | how an agent-produced image opens when you uppercase-hint its path: `auto` (real pixels on kitty and Ghostty, block glyphs everywhere else — the default), `kitty` (attempt pixels even where the terminal isn't recognised; does **not** yet make tmux work — the payload is not wrapped in tmux's passthrough envelope), `glyph` (never attempt pixels), `off` (no overlay; hinting an image path just copies it) |
