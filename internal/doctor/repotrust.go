...
// grant with its repo's current committed config. The returned error is a
// ledger that could not be read (corrupt, or written by a newer atrium) —
// worth a section of its own, because every repo reads as untrusted while it
// stands and nothing in the TUI says so persistently.
//
...
// repo costs one timeout, not a hang — bounded by how many repos are trusted.
func CheckRepoTrust(ctx context.Context) ([]RepoTrustEntry, error) {
	...
	for _, key := range keys {
		...
		entries = append(entries, RepoTrustEntry{
			Key:     key,
			State:   repotrust.LiveState(ctx, key, rec),
...
func RenderRepoTrust(entries []RepoTrustEntry, ledgerErr error) string {
	...
	for _, e := range entries {
		fmt.Fprintf(&b, "  %-10s %-22s granted %s  %s\n", e.Hash, e.State, e.Granted, e.Key)
	}
	return b.String()
}
