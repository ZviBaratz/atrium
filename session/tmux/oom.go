...
// agentOOMMargin is the process-wide default OOM margin baked into every session's
// launch command (see wrapOOMScore). It is a package var rather than a per-session
// parameter because the margin is one global policy — config.GetAgentOOMMargin —
// that every session-creation path (new, resume, restore) must pick up uniformly;
// the app resolves it once at startup via SetAgentOOMMargin. A session may still
// override it before Start with SetOOMScoreMargin. Its zero value is the disabled
// state, so a process that never sets it is safely unprotected rather than broken.
// It is atomic because the Settings panel may re-set it on the UI thread while a
// session Start reads it on a background goroutine.
var agentOOMMargin atomic.Int64

// SetAgentOOMMargin sets the process-wide default OOM margin new sessions inherit.
// Call once at startup (from config.GetAgentOOMMargin) and again when the setting
// changes; later creation paths read the value through newSession.
