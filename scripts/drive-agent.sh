...
#
# NOT isolated BY DEFAULT: the agent's own config dir. agy authenticates through
# ~/.gemini/antigravity-cli and codex through its own; a sandboxed HOME would leave
# both signed out, and driving a LIVE, AUTHENTICATED CLI is the entire point.
#
# ATR_CAP_ENV is the opt-in that changes this per run, for a CLI that offers a variable
# for the job — gemini's GEMINI_CLI_HOME is why it exists (#736), because everything
# past that CLI's trust dialog was otherwise unreachable without driving against the
# real ~/.gemini. See `help`'s ISOLATING AN AGENT'S CONFIG DIR, and note that it hands
# the CLI a variable rather than making the CLI honour one. Three consequences you
# accept by running this WITHOUT it:
#
...
#
# The pane's extra environment (ATR_CAP_ENV) is deliberately NOT a meta.env field. It
# lives in its own file, $RUN/cap-env, for the same reason the fleet snapshot does: it
# is a LIST, and meta.env is sourced by every verb, so a multi-line value there would
# have to survive a round trip through shell quoting on every single call. A separate
# file also lets load_cap_env DIE when it is missing, which is what turns "this run
# directory predates ATR_CAP_ENV" from a silent isolation loss into a stop.
CAP_ENV_ARGS=() # `-e NAME=VALUE` pairs for new-session
CAP_ENV_BARE=() # the same NAME=VALUE entries, for env(1) in probe_version
#
...
# ── the pane's extra environment (ATR_CAP_ENV) ───────────────────────────────────
#
# WHAT THIS IS FOR, AND WHAT IT IS NOT
#
# The file header says the agent's own config dir is deliberately NOT isolated, and
# that is still the default. This is the opt-in that lets a caller isolate it when the
# CLI offers a variable for the job — gemini has GEMINI_CLI_HOME (#736), and reaching
# its auth screen or its tool-confirmation dialog otherwise means driving against the
# real ~/.gemini. See `help`'s ISOLATING AN AGENT'S CONFIG DIR.
#
# It passes a variable. It does NOT make a CLI honour one — that is a per-agent fact,
# and the burden is on whoever writes a recipe to cite where the CLI reads it and to
# PROVE it landed. `up sh` + `send 'printenv …'` costs nothing and runs no agent.
#
# WHY NOT AN `env FOO=bar <bin>` PROGRAM STRING, which needs no code at all: cmd_up
# takes bin0="${program%% *}" and stamps BIN and `$BIN --version` into meta.env, which
# emit prints as every fixture's provenance. That spelling therefore records BIN=env
# and coreutils' version as the provenance of a gemini capture. #737 drove with it and
...
validate_cap_env() {
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		# caller set it INSIDE the pane points a nested tmux at the live fleet, which
		# is the accident every -S in this file exists to prevent. TMUX and TMUX_PANE
		# are the harness's for the same reason: they tell a program it is already
		# inside a session, and the pane's own identity is what every capture targets.
		case "$name" in
		TMUX | TMUX_TMPDIR | TMUX_PANE)
			die "ATR_CAP_ENV must not set $name — that variable belongs to the harness, not the agent"
			;;
		esac
		# A later -e silently wins, so a duplicate makes the run's environment a thing
		# you have to know tmux's precedence to read. An ambiguous provenance is not a
		# provenance.
		case "$seen" in
...
write_cap_env() {
	local line
	: >"$RUN/cap-env"
	chmod 600 "$RUN/cap-env"
	while IFS= read -r line; do
		[[ -n "$line" && "$line" != '#'* ]] || continue
		printf '%s\n' "$line"
	done <<<"${ATR_CAP_ENV:-}" >>"$RUN/cap-env"
}

# load_cap_env populates CAP_ENV_ARGS/CAP_ENV_BARE from the run's record. Called by
# start_session, so `fresh` re-applies the same environment on every rung — a fresh
# that dropped it would write fixtures taken against the real config dir while their
# headers claim the isolated one, which is the failure this whole mechanism is about.
load_cap_env() {
	CAP_ENV_ARGS=()
	CAP_ENV_BARE=()
	[[ -f "$RUN/cap-env" ]] ||
...
# cap_env_names prints the NAMES only, comma-separated, for emit and status. Names,
# never values: a value here is a path into someone's home and one of them points at a
...
# the context bar ON (GetSessionContextBar in the config package defaults true),
...
probe_version() {
	...
	# Under the run's OWN environment, or the recorded version is the version of a
	# differently-configured binary. Not hypothetical: gemini reads GEMINI_CLI_HOME and
	# parses settings.json out of it in its relaunch wrapper, before it prints anything.
	#
	# `env` execs in the SAME process, so the invariant the next comment protects
	# survives it: $! still names the process that becomes the binary, and the
	# SIGTERM-then-SIGKILL escalation below still lands on it rather than on a wrapper.
	#
	...
	env ${CAP_ENV_BARE[@]+"${CAP_ENV_BARE[@]}"} "$bin" --version </dev/null >"$out" 2>/dev/null &
	pid=$!
...
start_session() {
	...
	load_cap_env
	# new-session -e is tmux 3.0 ("Add -e flag for new-session to set environment
	# variables, like the same flag for new-window"), two minors below MIN_TMUX, so
	# this does not move the floor drive_agent_drift_test.go pins.
	#
...
cmd_up() {
	...
	write_capture_conf
	# Same timing, same reason: start_session reads this back through load_cap_env, and
	# `fresh` reads it again on every rung.
	write_cap_env
...
# noenter_check reports whether NOENTER=1 is in force, and refuses a text that would
# submit anyway. It exists for #735's fixture: a composer TALLER THAN THE PANE, which
# is an UNSUBMITTED state, and `send`/`paste` both end in Enter so neither could reach
# it. `keys` cannot either — without -l every argument is read as a key NAME.
#
# Skipping guard_enter under NOENTER is sound only because nothing else can submit,
# which is exactly what the refusal below enforces: send-keys -l sends those bytes RAW,
# and 0x0A is C-j, which sends_enter already counts as a submit for this reason. So the
# guard is dropped and the mechanism that made it necessary is removed in the same
# breath, rather than dropped on trust.
noenter_check() {
	[[ "${NOENTER:-0}" == "1" ]] || return 1
	[[ "$1" != *$'\n'* && "$1" != *$'\r'* ]] ||
		die "NOENTER=1 refuses a text containing a newline or carriage return — send-keys -l delivers those bytes raw, and 0x0A submits"
	return 0
}
...
cmd_emit() {
	...
	local f base label width frame name body redacted cap_names
	...
	for f in "${files[@]}"; do
		...
		# A capture taken under a non-default agent config is not the same artifact as
		# one taken under the default, and nothing else on the fixture would say so.
		# NAMES only: a value here is a path into someone's home, and one of them
		# points at a credentials directory — the same call the email redaction above
		# makes. Which config those names selected is the fixture header's job to state.
		if [[ -n "$cap_names" ]]; then
			printf '// Driven with ATR_CAP_ENV: %s.\n' "$cap_names"
		fi
...
cmd_status() {
	...
	printf 'run      %s\n' "$RUN"
	printf 'program  %s (%s)\n' "$PROGRAM" "$VERSION"
	# Mid-run you need to know not just which run you are steering but under what
	# environment — a `fresh` rung inherits it, and the answer decides whether the
	# capture is of the isolated config or the real one. Names, never values (cap_env_names).
	local cap_names
	cap_names="$(cap_env_names)"
...
ENV
  ...
  ATR_CAP_ENV    newline-separated NAME=VALUE lines handed to the pane via
...
ISOLATING AN AGENT'S CONFIG DIR
  ...
  authenticated CLI is the point (see the file header for the three costs you accept).

  ATR_CAP_ENV is the opt-in for when a CLI offers a variable for the job. It passes a
  variable; it does NOT make a CLI honour one. That is a per-agent fact, so a recipe
  owes a citation for where the CLI reads it — and a PROOF that it landed:

      ATR_CAP_NAME=envproof ATR_CAP_ENV='FOO=bar' scripts/drive-agent.sh up sh
      scripts/drive-agent.sh send 'printenv FOO'      # no agent runs, nothing at risk

  gemini (#736), which needs an isolated config dir to reach anything past the trust
  dialog — its auth screen, its idle composer, its tool-confirmation dialog:

      CAPHOME=/some/scratch/gemini-cli-home           # mode 0700
      export ATR_CAP_ENV=$'GEMINI_CLI_HOME='"$CAPHOME"$'\nGEMINI_FORCE_FILE_STORAGE=true'

  GEMINI_CLI_HOME: homedir() returns it when set and the CLI creates .gemini inside it
  (bundle chunk-TBDX7VEE.js; docs/reference/configuration.md). GEMINI_FORCE_FILE_STORAGE
  closes the hole GEMINI_CLI_HOME alone leaves: credentials go through HybridTokenStorage,
  which tries the OS keychain FIRST, and that store is global — no HOME-shaped variable
  can redirect it.

  Read this before you use that pair: forcing file storage WIDENS the blast radius of a
  failed passthrough. loadCredentials() falls through to migrateFromFileStorage(), which
  reads homedir()/.gemini/oauth_creds.json and then DELETES it. If the variables do not
  reach the child, that is your real refresh token. Prove the passthrough with `up sh`
  above — no gemini process — and back the file up, before gemini runs even once.
