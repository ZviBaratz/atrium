...
// CommandPaletteSize: three columns wide (key, verb, prose) and as many rows
// as it can get — the palette's whole value is seeing a lot of the keymap at
// once — capped like the command log so a very wide terminal doesn't stretch
// the prose column past comfortable reading. The share is of the box, border
// and padding included, so it is the room the palette may occupy rather than
// the room it fills and then overruns; HExtra keeps the rendered size where
// it was before that was true, and Fit's terminal clamp is what keeps the
// overhang inside a short terminal.
